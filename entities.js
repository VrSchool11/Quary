...
const HUM_TIERS = [
  { min: 76, cls: 'good', line: 'a normal human' },
  { min: 40, cls: 'warn', line: 'A potentail hazard' },
  { min: 0, cls: 'danger', line: 'Bialogical Harzad' },
];
let cacheKit = '',
  cacheGun = '',
  cachePrompt = '',
...
function endNote(won, hum, mark) {
  ...
  if (!won && game.quietKills === 0 && game.kills > 0)
    return 'You fought them head on every time. Try getting behind them.';
  if (!won && !p.fernFrames) return 'Never used the ferns. Guards cant see you in there.';
  if (!won) return 'Closer this time. Try again.';
  if (!game.ghosted.length)
    return 'Guards spotted you in every zone. Try slipping through one clean.';
  if (game.fitted < 3) return `There are more limbs out there. You only fitted ${game.fitted}.`;
  if (hum < 40) return 'Try coming out with more of yourself intact.';
  if (mark.faster && mark.prev)
    return `${mark.prev.secs - mark.secs} seconds faster than last time.`;
  if (mark.prev) return `Fastest so far is ${mark.prev.secs} seconds. Beat it.`;
  return 'Now do it faster.';
}
function escapeRank(hum) {
  if (hum >= 85) return 'Still you';
  if (hum >= 60) return 'Mostly you';
  if (hum >= 35) return 'Not fully you';
...
function endRun(won) {
  ...
  $('go-sub').textContent = won
    ? hum > 75
      ? 'You made it out looking like yourself.'
      : hum >= 40
        ? 'Mostly you walked out.'
        : 'Something walked out. It used to be you.'
    : 'They dragged you back and took what was theirs.';
  ...
  rows.push([
    ...
    won
      ? 'past the fence'
      : `in ${ZONES[game.zone].name}, ${Math.round((game.player.x / zoneExit) * 100)}% of the way through`,
  ]);
  rows.push(['Limbs fitted', String(game.fitted)]);
  rows.push([
    'Left unseen',
    game.ghosted.length ? `${game.ghosted.length} of ${ZONES.length} zones` : 'seen every time',
  ]);
  rows.push([
    'Killed',
    game.kills
  ...
  if (game.player.gun) {
    rows.push([
      'Rounds spent',
      game.player.fired + (game.player.silencer ? ' through a silencer' : ', all of them loud'),
    ]);
  }
  rows.push(['Took', secs + ' seconds' + (faster && prev ? ' — fastest yet' : '')]);
  if (better) {
    rows.push(['Best escape', Math.round(hum) + ' — new best']);
  } else if (prev) {
    rows.push(['Best escape', prev.hum + ' — still your best']);
  }

  $('go-summary').innerHTML = rows
    .map(
      ([k, v]) =>
        `<dt>${k}</dt><dd${/none|every time/.test(v) ? ' class="quiet"' : ''}>${v}</dd>`,
    )
    .join('');
  $('go-note').textContent = endNote(won, hum, { prev, secs, faster });
  const resume = $('btn-resume-cp');
  if (!won && game.checkpoint) {
    resume.textContent = 'Continue at ' + ZONES[game.checkpoint.zone].name;
...
const TIPS = [
  'stand in the ferns. they cant see it.',
  'attack from behind to prevent guard being alterted',
  '<b>Q</b> swaps the side a limb attaches to',
  'five rounds a mag',
  'the alarm dies down once you are out of sight',
  'frog legs give you better jump but cause guard to be more alerted',
  'sleeve, spring, rag, then hold <b>C</b> to make a silencer',
  'you need a human limb to fire the gun',
];
let cacheSector = '',
  tipBag = [],
  tipT = 0;
...
function updateHud() {
  ...
  if (p.gun) {
    ...
    if (gunLine !== cacheGun) {
      ...
      const tag = $('gun-tag');
      if (p.silencer) {
        tag.textContent = 'silenced';
        tag.className = 'gun-tag quiet';
      } else {
        tag.textContent = 'Loud';
